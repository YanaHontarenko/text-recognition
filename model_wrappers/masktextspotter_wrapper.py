...
class MaskTextSpotterWrapper:
    ...
    def detect_and_recognize(self, image):
        start = time()
        result_polygons, result_words = self.run_on_opencv_image(image)
        self.visualization(image, result_polygons, result_words)
        return image, time() - start


if __name__ == "__main__":
    text_spotter = MaskTextSpotterWrapper(os.path.join("data", "masktextspotter-config", "finetune.yaml"))
    image = cv2.imread(os.path.join("data", "images-to-test", "test-all.jpg"))
    image, _ = text_spotter.detect_and_recognize(image)
